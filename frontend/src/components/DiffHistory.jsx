 ...
 * DiffHistory.jsx
 *
 * List of past (and in-progress) diff computations for the active site.
 * Uses .dh-* CSS classes from viewer.css.
 */

// Backend sends naive timestamps like "2026-06-23T01:47:17.770" with no
// timezone — JS treats those as local time, which is wrong since the backend
// clock is UTC. Appending "Z" corrects this everywhere.
...
// < 1h: relative ("5분 전"). Today: wall-clock time. Older: "Jun 12".
// Absolute time/date past the 1h mark so clock skew can't show a confusing
// "X시간 전" for something that just finished.
...
// Pull {y, m, d} out of common label formats:
...
// Compact "YY.MM.DD → YY.MM.DD". Falls back to raw label if it doesn't parse.
...
export default function DiffHistory({ entries, activeId, onLoad, onDelete, onCancel, pollingIds, deletingIds, cancellingIds, splitMode, activeIdB }) {
  ...
  if (!visible.length) {
    return <div className="no-dates">변화탐지 기록이 없습니다</div>
  }
    ...
    <div className="dh-list">
      {visible.map(entry => {
        ...
        return (
          ...
          >
            {/* In split mode, show which slot (A/B) this entry occupies */}
            ...
            {/* Status badge — overridden by "삭제 중"/"취소 중" while mid-action */}
            <div className="dh-status-col">
              {isDeleting ? (
                ...
                <>
                  {isSucceeded && <span className="vst-badge vst-done">완료</span>}
                  {isQueued    && <span className="vst-badge vst-running">대기 중</span>}
                  {isRunning   && <span className="vst-badge vst-running">생성 중</span>}
                </>
              )}
            ...
            {/* ✕ — delete for a finished row, cancel for one still in progress. Once clicked, swaps to a disabled blank state so
                a second click can't fire a second request. */}
            {isSucceeded && (
              <button
